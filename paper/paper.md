...
species_i: ith-species; 36 specifies the number of species in the ``CBMZ-MOSAIC`` chemical mecanism (remember to use the right units: $mol km^{-2}^ hr^{-1}^$ for gases and $&mu;g m^{-2}^ s^{-1}^$ for aerosols). Complete with columns of ``0`` if data is not available.
